...
export async function updatePost(req: Request, res: Response) {
  const post = await postService.updateBlogPost(paramInt(req.params.id), req.body);
  return ApiResponse.ok(res, post, "پست وبلاگ به‌روزرسانی شد");
}

export async function removePost(req: Request, res: Response) {
  await postService.deleteBlogPost(paramInt(req.params.id));
  return ApiResponse.ok(res, null, "پست وبلاگ حذف شد");
}

export async function getBySlugPublic(req: Request, res: Response) {
  return ApiResponse.ok(res, await postService.getBlogPostBySlugPublic(typeof req.params.slug === "string" ? req.params.slug : ""));
}

export async function getByIdAdmin(req: Request, res: Response) {
  return ApiResponse.ok(res, await postService.getBlogPostByIdAdmin(paramInt(req.params.id)));
}
...
export async function updateCategory(req: Request, res: Response) {
  const category = await categoryService.updateBlogCategory(paramInt(req.params.id), req.body);
...
export async function removeCategory(req: Request, res: Response) {
  await categoryService.deleteBlogCategory(paramInt(req.params.id));
