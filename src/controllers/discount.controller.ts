...
export async function update(req: Request, res: Response) {
  const discountCode = await discountService.updateDiscountCode(paramInt(req.params.id), req.body);
  return ApiResponse.ok(res, discountCode, "کد تخفیف به‌روزرسانی شد");
}

export async function remove(req: Request, res: Response) {
  await discountService.deleteDiscountCode(paramInt(req.params.id));
  return ApiResponse.ok(res, null, "کد تخفیف حذف شد");
}

export async function getById(req: Request, res: Response) {
  return ApiResponse.ok(res, await discountService.getDiscountCodeById(paramInt(req.params.id)));
}
