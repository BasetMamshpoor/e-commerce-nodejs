...
export async function requestWithdrawal(
  ...
  amount: number,
  description?: string,
  bankInfo?: { bankSheba?: string; bankCardNumber?: string; bankAccountOwnerName?: string }
) {
  ...
  return prisma.walletWithdrawal.create({
    data: {
      ...
      description,
      status: "PENDING",
      bankSheba: bankInfo?.bankSheba,
      bankCardNumber: bankInfo?.bankCardNumber,
      bankAccountOwnerName: bankInfo?.bankAccountOwnerName,
    },
  });
}
...
export async function reviewWithdrawalAdmin(
  withdrawalId: number,
  ...
  if (status === "APPROVED") {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      await tx.walletWithdrawal.update({ where: { id: withdrawalId }, data: { status: "APPROVED", adminNote, trackingCode, reviewedAt: new Date() } });
    });
    ...
    await prisma.walletWithdrawal.update({ where: { id: withdrawalId }, data: { status: "REJECTED", adminNote, trackingCode, reviewedAt: new Date() } });
